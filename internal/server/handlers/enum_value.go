...
// @Description Создает новое значение для указанного перечисления
// @Tags enumeration-values
// @Accept json
// @Produce json
// @Param request body dto.EnumValueCreateRequest true "Данные для создания значения"
// @Success 201 {object} dto.EnumValueDTO "Созданное значение перечисления"
// @Failure 400 {object} dto.ErrorResponse "Ошибка валидации"
// @Failure 404 {object} dto.ErrorResponse "Родительское перечисление не найдено"
// @Failure 409 {object} dto.ErrorResponse "Значение с таким кодом уже существует"
...
// GetAll возвращает значение перечисления
// @Summary Получить значения
// @Description Возвращает значение перечисления по его идентификатору
// @Tags enumeration-values
// @Produce json
// @Success 200 {object} dto.EnumValueDTO "Найденное значение"
// @Failure 400 {object} dto.ErrorResponse "Неверный формат ID"
// @Failure 404 {object} dto.ErrorResponse "Значение не найдено"
// @Failure 500 {object} dto.ErrorResponse "Внутренняя ошибка сервера"
...
// GetByEnumId возвращает все значения для перечисления
...
// // Update обновляет значение перечисления
// // @Summary Обновить значение перечисления
// // @Description Обновляет существующее значение перечисления
// // @Tags enumeration-values
// // @Accept json
// // @Produce json
// // @Param request body dto.EnumUpdateRequest true "Данные для обновления"
// // @Success 204 "Значение успешно обновлено"
// // @Failure 400 {object} dto.ErrorResponse "Ошибка валидации"
// // @Failure 404 {object} dto.ErrorResponse "Значение не найдено"
// // @Failure 409 {object} dto.ErrorResponse "Конфликт данных (код уже существует)"
// // @Failure 500 {object} dto.ErrorResponse "Внутренняя ошибка сервера"
// // @Router /enumeration-values [put]
// func (h *EnumValueHandler) Update(w http.ResponseWriter, r *http.Request) {
// 	ctx := r.Context()
...
// 		Label: req.Label,
// 	}
// 	_, err := h.enumValueService.Update(ctx, enumValue)
// 	if err != nil {
// 		middleware.HandleError(w, r, err)
// 		return
// 	}
...
// @Description Удаляет значение перечисления (мягкое или полное удаление)
// @Tags enumeration-values
// @Produce json
// @Param id path int true "ID значения перечисления"
// @Param soft query bool false "Мягкое удаление (true/false)" default(true)
// @Success 204 "Значение успешно удалено"
